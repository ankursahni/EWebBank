...
public class UserDaoImpl implements IuserDao {
	...
	static {
		try {
			...
			conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521/XE", "system", "ankur");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public boolean userLogin(int acc, String password) {
		...
		try {
			PreparedStatement pstmt = conn.prepareStatement("select password from users where accno=?");
			pstmt.setInt(1, acc);
	...
	public double deposit(int acc, double amount) {
		...
		try {
			double nmt = amount;
			PreparedStatement pstmt = conn.prepareStatement("select balance from users where accno=?");
			pstmt.setInt(1, acc);
			ResultSet rset = pstmt.executeQuery();
			rset.next();
			...
			PreparedStatement pstmt1 = conn.prepareStatement("update users set balance=? where accno=?");
			pstmt1.setDouble(1, amt);
			pstmt1.setInt(2, acc);
			pstmt1.execute();

			PreparedStatement stmt = conn.prepareStatement("select transcount from users where accno=?");
			stmt.setInt(1, acc);
			...
			case 0:
				PreparedStatement pstmt2 = conn
						.prepareStatement("update users set transcount=? , trans1=? where accno=?");
				pstmt2.setInt(1, 1);
			...
			case 1:
				PreparedStatement pstmt3 = conn
						.prepareStatement("update users set transcount=? , trans2=? where accno=?");
				pstmt3.setInt(1, 2);
			...
			case 2:
				PreparedStatement pstmt4 = conn
						.prepareStatement("update users set transcount=? , trans3=? where accno=?");
				pstmt4.setInt(1, 3);
			...
			case 3:
				PreparedStatement pstmt5 = conn
						.prepareStatement("update users set transcount=? , trans4=? where accno=?");
				pstmt5.setInt(1, 4);
			...
			case 4:
				PreparedStatement pstmt6 = conn
						.prepareStatement("update users set transcount=? , trans5=? where accno=?");
				pstmt6.setInt(1, 0);
	...
	public List<Double> statement(int accno) {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("select trans1,trans2,trans3,trans4,transcount from users where accno=?");
			pstmt.setInt(1, accno);
			ResultSet rset = pstmt.executeQuery();
			rset.next();
			...
			// the below code is to print statement in correct transaction order
			// of actions

			int temp = rset.getInt(5);
	...
	public void changePassword(int accno, String Password) {
		try {
			PreparedStatement pstmt = conn.prepareStatement("update users set password=? where accno=?");
