...
public class AdminDaoImpl implements IadminDao {
	...
	@Override
	public boolean addUser(int acc, String name, double balance) {
	...
	public BankUser userDetails(int accno) {
		try {
			PreparedStatement pstmt =conn.prepareStatement("select name,balance from users where accno=?");
	...
	public boolean modifyUser(int accno, String name) {
		try {
			PreparedStatement pstmt =conn.prepareStatement("update users set name=? where accno=?");
	...
	public List<BankUser> showAll() 
	{
		try {
			...
			ResultSet rset = stmt.executeQuery("select accno,name,balance from users");
			while(rset.next())
			{
